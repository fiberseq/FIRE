...
# correct DNAse signal with GC content
```{r}
...
```{r}
df %>% filter(end-start > 300, acc_percent > 0.85) %>%
    rename(`#chrom`=chrom) %>% 
    fwrite("tmp.bed", sep="\t")
dnase_peaks_with_gc = fread(cmd="bedtools nuc -bed tmp.bed -fi ~/assemblies/hg38.analysisSet.fa")
colnames(dnase_peaks_with_gc)[1:length(colnames(df))] = colnames(df)
dnase_peaks_with_gc$GC_frac = dnase_peaks_with_gc$`42_pct_gc` 
```

```{r}
colnames(dnase_peaks_with_gc)
z=dnase_peaks_with_gc %>% 
    ggplot(., aes(x=`42_pct_gc`, y=dnase_max)) +
        geom_hex(bins=50) + 
        geom_smooth(se=F)+
        scale_fill_distiller("", 
            palette = "Spectral",
...
```{r}
tFDR=0.01
peaks_to_sample = df %>% filter(FDR < tFDR) %>% group_by(group) %>% sample_n(200) %>% select(group,ID) %>% data.table
