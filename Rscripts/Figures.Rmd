...
make_hap_plots = function(SM, show_imprinted=FALSE, p_threshold = 0.05){
    ...
    n=comma(nrow(tdf))
    p = tdf %>%
        ggplot(aes(x=diff, y=-log10(p_value))) +
        geom_hex(bins=100) + scale_fill_distiller("", palette = "Spectral", trans="log10") +
        geom_hline(aes(yintercept=-log10(p_threshold)), linetype="dashed", color="darkblue")+
        geom_hline(aes(yintercept=-log10(cor_p_threshold)), linetype="dashed", color="darkred")+
        facet_wrap(~autosome, ncol=2)+
        scale_x_continuous("Difference between paternal and maternal accessibility", labels=percent) +
        scale_y_continuous(glue("-log10[ adjusted p-value ]    (n = {n})"), labels=comma) +
        my_grid()
    if(show_imprinted){
        p + geom_point(
            data=tdf%>% 
                filter(imprinted>0, p_value <= 0.05),
            aes(x=diff, y=-log10(p_value)),
            shape="+",
            #alpha=0.75,
            color="darkred",
        )
...
pdf %>% 
    arrange(p_cor) %>% 
    fwrite(glue("Tables/{SM}_hap1-vs-hap2-accessability.tsv.gz"), sep="\t")
```
