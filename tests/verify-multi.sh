...
# the chr20-only sample must have no chr21 peaks. grep -c reads the
# whole stream, so SIGPIPE cannot mask a hit under pipefail.
...
# test-rev holds the same reads as test, with a reversed header. Output
# content must therefore be identical after a row sort. This is the one
# assertion that can catch a silent ordering bug.
assert_same_content() {
